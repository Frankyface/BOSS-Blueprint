/**
 * Fixed boilerplate regions — §3.3 rule 1: "'Your role' and 'Responsive rules'
 * are fixed boilerplate (they interpolate nothing but constants)". The
 * Definition-of-done region is likewise constant.
 *
 * These are frozen constants, not templates: feature-brief-generator.md's
 * equality test C asserts each byte-matches its §3.2 counterpart in
 * docs/export-format.md, so a wording change in the spec fails CI instead of
 * silently shipping a stale prompt.
 *
 * Each array element is ONE logical line (§3.3 rule 10 — no hard wrapping);
 * '' is a blank line.
 */
...
/** §3.2 — fixed fallbacks for absent optionals (§3.3 rule 4). */
